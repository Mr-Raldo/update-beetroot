...
function transformString(inputString) {
  // Remove underscores
  let stringWithoutUnderscore = inputString.replace(/_/g, ' ');

  // Capitalize the first character
  let capitalizedString = stringWithoutUnderscore.charAt(0).toUpperCase() + stringWithoutUnderscore.slice(1);
...
$(function () {
  ...
  var dt_user_table = $('.datatables-users'),
    select2 = $('.select2'),
    userView = '/akyc/profiles/details/overview/',
    statusObj = {
  ...
  if (dt_user_table.length) {
    var dt_user = dt_user_table.DataTable({
      ajax: "/akyc/profiles/list/", // JSON file to add data
      columns: [
      ...
      columnDefs: [
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            console.log('full profile',full['profile'])
            console.log('wallet', full['wallet'])
            var profile = full['profile']
            var $id = profile['id'];
            var $name = profile['first_name'] + ' ' + profile['last_name'],
                $email = profile['email'],
                $role = full['role'] == null ? '' :  full['role'],
                $image = profile['profile_image'],
                $account_type = profile['account_type'] == null ? '' : full['account_type'],
                $phone = full['phone']
            if ($image) {
              // For Avatar image
              var $output =
            ...
            var $row_output =
              ...
              transformString(profile['first_name']) + ' ' + transformString(profile['last_name']) +
              '</span></a>' +
              '<small>' +
              $id +
              '</small>' +
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            ...
            var $role = profile['account_type'] == null?'User':transformString(profile['account_type']);
            var roleBadgeObj = {
              ...
              Provider: '<i class="mdi mdi-store mdi-20px text-success me-2"></i>'
            };
        ...
        },
        {
          ...
          render: function (data, type, full, meta) {
            var profile = full['profile']
            var $specialization = profile['specialization'] != null ? transformString(profile['specialization']): 'Client';
            $specialization = $specialization.replace(/_/g, " ")
            return '<span class="text-heading">' + $specialization + '</span>';
          }
        },
        {
          // Wallet Balance
          ...
          render: function (data, type, full, meta) {
            ...
            var currentBalance = wallet['currentBalance'] != null ? wallet['currentBalance'] : 0;
            console.log('current Balance')
            console.log(currentBalance)
            return '<span class="text-heading">' + currentBalance  + '</span>';
          }
        },
        {
          // User Status
          ...
          render: function (data, type, full, meta) {
            var profile = full['profile']
            var wallet = profile['trading_as'];

            return (
              '<span class="badge rounded-pill ' +
              profile['trading_as'] != null ? profile['trading_as'] : '' +
              profile['trading_as'] != null ? profile['trading_as'] : '' +
              '" text-capitalized>' +
              profile['trading_as'] != null ? profile['trading_as'] : '' +
              '</span>'
      ...
      initComplete: function () {
        ...
        this.api()
          ...
          .every(function () {
            var column = this;
            var select = $(
              '<select id="AccountType" class="form-select text-capitalize"><option value=""> Select Account Type </option></select>'
            )
              .appendTo('.user_role')
              .on('change', function () {
        ...
        this.api()
          ...
          .every(function () {
            var column = this;
            var select = $(
              '<select id="UserPlan" class="form-select text-capitalize"><option value=""> Select Specialization </option></select>'
            )
              .appendTo('.user_plan')
              .on('change', function () {
                var val = $.fn.dataTable.util.escapeRegex($(this).val());
        ...
        this.api()
          ...
          .every(function () {
            var column = this;
            var select = $(
              '<select id="FilterTransaction" class="form-select text-capitalize"><option value=""> Select Trade </option></select>'
            )
